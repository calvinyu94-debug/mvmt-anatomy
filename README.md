...
| [`tools/bp3d_export.py`](tools/bp3d_export.py) | Carries fascia, ligaments, insertions, nerves, landmarks and the muscles BodyParts3D lacks onto the BP3D body in Human Atlas's chunk format, one chunk per region; projects insertions onto their own BP3D bone; authors the schematic spinal cord, roots, cauda and dura through the BP3D canal; judges every part's fit by its system's rule; the seam check; the review renders |
